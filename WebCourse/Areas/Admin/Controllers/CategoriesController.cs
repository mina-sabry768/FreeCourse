...
namespace WebCourse.Areas.Admin.Controllers
{
    ...
    public class CategoriesController : Controller
    {
        ...
        public CategoriesController(IServicesRepository<Category> servicesCategory, IServicesRepositoryLog<LogCategory> servicesCategoryLog, UserManager<ApplicationUser> userManager)
        {
        ...
        public IActionResult Save(CategoryViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.NewCategory.Id == Guid.Parse(Guid.Empty.ToString()))
                {
                    ...
                    {
                        if (_servicesCategory.Save(model.NewCategory) && _servicesCategoryLog.Save(model.NewCategory.Id, Guid.Parse(userId)))
                ...
                {
                    //Update
                    if (_servicesCategory.Save(model.NewCategory) && _servicesCategoryLog.Update(model.NewCategory.Id, Guid.Parse(userId)))
